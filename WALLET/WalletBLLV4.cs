...
namespace WALLET
{
    public class WalletBLLV4
    {
        ...
        public bool Transferencia(string key, TransferenciaRequest Request, ref TransferenciaResponse Response)
        {
            ...
            try
            {
                ...
                if (oRem != null)
                {
                    ...
                    cargo.Referencia = Request.referenciaNumerica.ToString();

                    ////Realiza abono en la wallet        
                    string request = JsonConvert.SerializeObject(cargo);
                    LogClass.LogInfo(key + "CrossBorderWeb", "RealizarAbonoWallet", "Wallet request: " + request);
                    ResponseCargo responseDataCargo = realizarAbono(key, "Transferencia", cargo);
                    string response = JsonConvert.SerializeObject(responseDataCargo);
                    LogClass.LogInfo(key + "CrossBorderWeb", "RealizarAbonoWallet", "Wallet response: " + response);
                    ...
                    oTransfer.key = key;
                    oTransfer.dMontoWallet = responseDataCargo.Saldo;

                    LogClass.LogInfo(key + "CrossBorderWeb", "Encolar Transferencia", $"Encolar Transferencia Id{Request.id} Referencia {cargo.Referencia} FolioWallet {responseDataCargo.Folio}");
                    Singleton.Instance.EncolarTransferencia(oTransfer);
                    //LogClass.LogInfo(key + "CrossBorderWeb", "Encolar Transferencia", $"Total Transferencias {Singleton.Instance.ColaTrannsferencia.Count}");
            ...
            {
                LogClass.LogError(key + "CrossBorderWeb", "WalletBLL Transferencia", "JsonException ex: " + ex.Message);
                Response.mensaje = ex.Message;
                return false;
            }
            catch (Exception ex)
            {
                LogClass.LogError(key + "CrossBorderWeb", "WalletBLL Transferencia", "Exception ex: " + ex.Message);
                Response.mensaje = ex.Message;
                return false;
            }
        }
        ...
        public RemitenteEntity ObtenerRemitenteSTP(string key, string CksReferencia)
        {
            ...
            try
            {
                LogClass.LogInfo(key + "CrossBorderWeb", "ObtenerRemitenteSTP", $"Referencia {CksReferencia} Inicia");
                ...
                _sqlCmd.Prepare();
                LogClass.LogInfo(key + "CrossBorderWeb", "ObtenerRemitenteSTP", $"sp_select_cuentastp Referencia {CksReferencia} Prepara comando");
                _sqlDatRead = _sqlCmd.ExecuteReader();
                LogClass.LogInfo(key + "CrossBorderWeb", "ObtenerRemitenteSTP", $"sp_select_cuentastp Referencia {CksReferencia} Ejecuta comando");
                ...
                if (_sqlDatRead.HasRows == true)
                {
                    ...
                    LogClass.LogInfo(key + "CrossBorderWeb", "ObtenerRemitenteSTP", $"sp_select_cuentastp Referencia {CksReferencia} Resultado=> nIdRemitente {rem.nIdRemitente} nNumCuenta {rem.nNumCuenta}");
                    _sqlDatRead.Close();
                }  
                else
                    LogClass.LogInfo(key + "CrossBorderWeb", "ObtenerRemitenteSTP", $"sp_select_cuentastp Referencia {CksReferencia} Resultado:=> Sin Resultados");

            }
            catch (Exception ex)
            {
                rem = null;
                LogClass.LogInfo(key + "CrossBorderWeb", "ObtenerRemitenteSTP", $"sp_select_cuentastp Referencia {CksReferencia} Error=> {ex.ToString()}");
            }
            finally
            {
            ...
            LogClass.LogInfo(key + "CrossBorderWeb", "ObtenerRemitenteSTP", $"Referencia {CksReferencia} Fin");
            return rem;
        }
        public ResponseCargo realizarAbono(string key, string tipo, Cargo cargo)
        {
            ...
            if (cargo.ClienteId != 0 && cargo.NumCuenta != 0 && cargo.TipoRecurso != 0 && !string.IsNullOrEmpty(cargo.Descripcion) && cargo.Monto != 0)
            {
                try
                {
                    ...
                    LogClass.LogInfo(key + "CrossBorderWeb", $"Abono WalletBLL {tipo}", $"Inicia Abono Referencia {cargo.Referencia}");
                    ...
                    {                        
                        ...
                        LogClass.LogInfo(key + "CrossBorderWeb", $"Abono WalletBLL {tipo}", $"Abono Exitoso Referencia {cargo.Referencia} Folio {response.Folio}");
                    }
                    catch (JsonException ex)
                    {
                        response.ErrorCode = (int)HttpStatusCode.BadGateway;
                        response.ErrorMsg = "Error al realizar cargo al servicio";
                        LogClass.LogInfo(key + "CrossBorderWeb", $"Abono WalletBLL {tipo}", $"Error al realizar cargo al servicio Referencia {cargo.Referencia}");
                    }                    
                }
                catch (Exception ex)
                {
                    ...
                    LogClass.LogInfo(key + "CrossBorderWeb", $"Abono WalletBLL {tipo}", $"Ocurrio un error al intentar consumir el servicio Wallet Referencia {cargo.Referencia} => {ex.Message}");
                }
            }
            else
            {
                ...
                LogClass.LogInfo(key + "CrossBorderWeb", $"Abono WalletBLL {tipo}", $"ClienteId, NumCuenta, TipoRecurso, Descripcion o Monto invalido, no puede ser 0 o vacio su valor Ref {cargo.Referencia}");
            }
            LogClass.LogInfo(key + "CrossBorderWeb", $"Abono WalletBLL {tipo}", $"Fin Abono Referencia {cargo.Referencia}");
            return response;
        }
